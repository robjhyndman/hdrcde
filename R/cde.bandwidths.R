...
"cdeband.rules0" <- function(
  ...
  if (xden == "uniform") {
    ...
    l <- min(x)
    if (!sdlinear) {
      a <- ((4 * sqrt(pi) * Rk^2 * (u - l) * abs(p)^5) /
        (3 * n * sk^4 * abs(d)^5))^(1 / 6)
      b <- abs(d) * a
    } else {
      pl <- junk$pl
      qx <- junk$qx
      z <- ((pl + qx * u)^4 - (pl + qx * l)^4) /
        ((pl + qx * u)^4 * (pl + qx * l)^4)
      w <- 19 * qx^4 + 4 * d^4 + 28 * d^2 * qx^2
      a <- ((2^(7.5) * sqrt(pi) * Rk^2 * (u - l)^2 * qx) /
        (3 * n * sk^4 * z * w^(0.75) * (sqrt(w) + 2 * d^2 - 3 * qx^2)))^(1 / 6)
      b <- a * w^(0.25) / sqrt(2)
    }
  } else if (xden == "normal") {
    erf <- 0.954499736
    ...
    if (!modified) {
      v <- 3 *
        ...
        sm^3 *
        pi -
        8 * sqrt(2 * pi) * p^2 * k * exp((-k^2) / 2) +
        8 * pi * p^2 * erf
      anum <- ((16 * Rk^2 * k * (pi)^(1.25) * p^5 * sm^(2.5)) /
        (n * abs(d)^(5 / 2) * sk^4))^(1 / 6)
      adem <- (((v^5) / (3 * (pi)^2 * sm^4 * erf))^(1 / 4) +
        3 * d * ((v * erf^(1 / 3)) / 3)^(3 / 4))^(1 / 6)
      a <- anum / adem
      ...
      adem <- n *
        sk^4 *
        abs(d)^2.5 *
        ((u^5 / (3 * sm^4))^0.25 + (3 * d^4 * u^3)^0.25)
      a <- (anum / adem)^(1 / 6)
...
"cdeband.rules" <- function(x, y, deg, link, mean.order, ...) {
  ...
  if (deg == 1) {
    ...
    if (link == "identity") {
      alpha <- (3 *
        d1^4 +
        36 * d2^2 * v^2 * (d1^2 + v^2 * d2^2) +
        8 * d2^2 * sigma^2) /
        (64 * pi * sigma^5 * v)
      beta <- 3 * (d1^2 + 2 * d2^2 * v^2) / (32 * pi * sigma^5 * v)
    } else {
      alpha <- (d1^4 +
        12 * d2^2 * v^2 * (d1^2 + v^2 * d2^2) +
        2 * d2^2 * sigma^2) /
        (16 * pi * sigma^5 * v)
...
cdeband.Mbh <- function(
  ...
  if (passes > 1 & !is.na(first$a)) {
    ...
    newa.grid <- seq(
      0.95 * first$a.grid[idx[1]],
      1.05 * first$a.grid[idx[length(idx)]],
      l = ngrid
    )
    second <- CDEband.Mbh(
      xx,
  ...
  if (!is.na(a) & usequad) {
    ...
    if (length(idx) > 3) {
      fit <- lm(
        q ~ a + I(a^2),
        data = data.frame(q = fullq[idx], a = fulla.grid[idx])
      )
...
"cdeband.bootstrap" <- function(
  ...
  fit1 <- lm(y ~ 1, data = df)
  aic[1] <- deviance(fit1) +
    2 * (n - fit1$df.residual) * deviance(fit1) / fit1$df.resid
  fit2 <- lm(y ~ x, data = df)
  aic[2] <- deviance(fit2) +
    2 * (n - fit2$df.residual) * deviance(fit2) / fit2$df.resid
  fit3 <- lm(y ~ x + I(x^2), data = df)
  aic[3] <- deviance(fit3) +
    2 * (n - fit3$df.residual) * deviance(fit3) / fit3$df.resid
  fit4 <- lm(y ~ x + I(x^2) + I(x^3), data = df)
  aic[4] <- deviance(fit4) +
    2 * (n - fit4$df.residual) * deviance(fit4) / fit4$df.resid
  ...
  if (na > 2 & nb > 2) {
    fit <- lm(
      imse ~ a + b + I(a^2) + I(b^2) + I(a * b),
      data = data.frame(
        imse = c(imse[idx.a, idx.b]),
        a = rep(a.grid[idx.a], nb),
        b = rep(b.grid[idx.b], rep(na, nb))
      )
    )
    best.b <- (fit$coef[2] * fit$coef[6] - 2 * fit$coef[3] * fit$coef[4]) /
      (4 * fit$coef[4] * fit$coef[5] - fit$coef[6] * fit$coef[6])
