...
hdrbw <- function(
  ...
  if (graphProgress) {
    ...
    plot(
      ...
      type = "l",
      bty = "l",
      main = expression(paste("Pilot estimate of ", f[HDRlevel])),
      xlab = "x",
  ...
  riskFuns <- function(s, B1, B2, B3, drv = 0) {
    ...
    if (drv == 0) {
      return(sum(
        (1 / s) * B1 * dnorm(farg) + B3 * (s^4) * (2 * pnorm(farg) - 1)
      ))
    }
    if (drv == 1) {
      return(sum(
        ((-1 / s^2) * B1 + 10 * B2 * B3 * (s^8) - 5 * B1 * (B2^2) * (s^8)) *
          dnorm(farg) +
    ...
    if (drv == 2) {
      return(sum(
        (2 *
          B1 *
          (1 / s^3) +
          5 * B2 * (24 * B3 - 7 * B1 * B2) * (s^7) -
          25 * (B2^3) * (2 * B3 - B1 * B2) * (s^17)) *
          dnorm(farg) +
  ...
  while (!finished) {
    ...
    fdds <- sum(
      (2 *
        B1hat *
        (1 / s^3) +
        5 * B2hat * (24 * B3hat - 7 * B1hat * B2hat) * (s^7) -
        25 * (B2hat^3) * (2 * B3hat - B1hat * B2hat) * (s^17)) *
        dnorm(farg) +
    ...
    sNew <- s - fds / fdds
    if (sNew < 0) {
      stop("Newton-Raphson failure (out of range).")
    }
    relErr <- abs((sNew - s) / s)
    if (relErr < tolerance) {
      finished <- TRUE
  ...
  if (graphProgress) {
    ...
    for (j in seq_along(B1hat)) {
      ...
      AriskGrid <- AriskGrid +
        B3hat[j] * (hgrid^2) * (2 * pnorm(B2hat[j] * sqrt(n * hgrid^5)) - 1)
    }
    plot(
      log(hgrid),
      AriskGrid,
