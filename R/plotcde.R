...
plot.cde <- function(
  ...
  for (i in 1:dimz[firstvar]) {
    ...
    fn(
      small.den,
      main = paste(x.name[firstvar], "=", xlabels[i]),
      mden = mden,
      ...
    )
  }
...
stacked.plot <- function(
  ...
  for (i in ylabels) {
    ...
    lines(perspp(c(xlim[1], xlim[1] + xrange / 75), c(i, i), c(0, 0), junk))
    text(
      perspp(xlim[1] - xrange / 20, i, 0, junk),
      paste(round(i, 4)),
      cex = 0.8,
      font = font
    )
  ...
  for (i in xlabels) {
    ...
    text(
      perspp(i, ylim[1] - yrange / 20, 0, junk),
      paste(round(i, 4)),
      cex = 0.8,
      font = font
    )
  ...
  text(
    perspp(midx, ylim[1] - yrange / 7, 0, junk),
    xlab,
    cex = 0.8,
    srt = 265,
    font = font
  ...
  text(
    perspp(xlim[1] - xrange / 7, midy, 0, junk),
    ylab,
    cex = 0.8,
    srt = 352,
    font = font
  ...
  for (i in index) {
    if (
      length(c(den$z[i, ])[!is.na(den$z[i, ])]) > 0 &
        mden[i] > threshold * maxden
    ) {
      pol <- perspp(
        rep(den$x[i], length(den$y) + 2),
...
hdr.cde <- function(
  ...
  if (plot.modes) {
    matpoints(
      x.margin[index],
      matrix(modes[index, ], nrow = sum(index)),
      cex = cex,
      col = 1,
      pch = 19
    )
