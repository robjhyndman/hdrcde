...
cde <- function(
  ...
  if (miss.a | miss.b) {
    ...
    if (miss.a) {
      if (fw) {
        ...
        a <- a.nndefault
      } ## Nearest neighbourhood span
    }
  }
  if (use.locfit) {
    if (fw) {
      ## For fixed bandwidth of a in locfit
      locfit.a <- c(0, 2.5 * a)
    } else {
      locfit.a <- a
    }
  ...
  for (i in seq_len(dim.cde[1])) {
    ...
    if (max(abs(newy)) < 1e-20) {
      ...
      fits <- try(
        predict(junk, newdata = as.matrix(x.margin.grid)),
        silent = TRUE
      )
  ...
  return(structure(
    list(
      ...
      deg = deg,
      link = link,
      fn = switch(use.locfit + 1, "ksmooth", "locfit"),
      x.name = x.name,
      y.name = y.name,
      fixed.width = fw,
      AIC = mean(AIC),
