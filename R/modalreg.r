...
modalreg <- function(
  ...
  if (missing(a) || missing(b)) {
    if (deg == 0) {
      if (!missing(a) || !missing(b)) {
        warning(
          "If either a or b is missing for deg=0, then both bandwidths are selected automatically."
        )
      }
      h <- cde.bandwidths(x, y, method = 1, deg = 0, ...)
    } else if (deg == 1) {
      stop(
        "No automatic bandwidth selection for deg=1. Specify the bandwidths by hand or choose deg=0 (recommended)."
      )
    }
    a <- h$a
    b <- h$b
    ...
    for (p in seq_len(P)) {
      ...
      for (j in seq_len(iter)) {
        ...
        if (deg == 1) {
          ...
          current.regression <- cond.meanshift(
            x,
            y,
            xfix[i],
            current.regression,
          ...
          stop("Polynomial degree not supported: Choose 0 or 1.")
        }
    ...
    for (p in seq_len(P)) {
      if (plot.type[1] == "p") {
        points(
          ...
          save.regression[p, ][kde[p, ] > Threshold],
          cex = 1,
          col = p +
            1,
          pch = alphabet[p]
...
cond.meanshift <- function(x, y, x0, y0, a, b) {
  sum(kern(x, x0, a) * gern(y, y0, b) * y) /
    (sum(kern(x, x0, a) * gern(y, y0, b)))
}

cond.linear.meanshift <- function(x, y, x0, y0, a, b) {
  sn1 <- sum(kern(x, x0, a) * (x0 - x))
  sn2 <- sum(kern(x, x0, a) * (x0 - x)^2)
  sum(kern(x, x0, a) * (sn2 - (x0 - x) * sn1) * gern(y, y0, b) * y) /
    (sum(kern(x, x0, a) * (sn2 - (x0 - x) * sn1) * gern(y, y0, b)))
}
